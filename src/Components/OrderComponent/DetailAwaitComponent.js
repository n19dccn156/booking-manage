...
import { ReloadAwaitTable } from '../../Actions/ReloadTableAction';
...
const DetailAwaitComponent = React.memo((props) => {
	...
	const confirmCancel = () => {
		...
		.then(() => {
			...
			setIsCancel(false);
			setIsOpen(false);
			dispatch(ReloadAwaitTable(!update));
		})
		.catch(() => {
	...
	const confirm = () => {
		axios({
			url: url+Constants.STATUS.COMFIRM,
			method: 'PUT',
			headers: {'Authorization': token, 'Content-Type': 'application/json'}
		})
		.then(() => {
	...
	return (
		<>
			...
			<Modal title="Bạn có đồng ý 'xác nhận đơn' ? " open={isConfirm} onCancel={notConfirm} onOk={confirm}></Modal>
		</>
	);
}
)
